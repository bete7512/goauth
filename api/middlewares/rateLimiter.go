...
func (m *Middleware) RateLimiterMiddleware(limiter config.RateLimiter, config *config.RateLimiterConfig, route string, next http.HandlerFunc) http.HandlerFunc {
...
func (m *Middleware) GlobalRateLimiterMiddleware(limiter config.RateLimiter, config *config.RateLimiterConfig, next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		clientIP := utils.GetIpFromRequest(r)
		if clientIP == "unknown" {
