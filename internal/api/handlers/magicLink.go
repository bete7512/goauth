...
func (h *AuthRoutes) SendMagicLink(w http.ResponseWriter, r *http.Request) {
...
func (h *AuthRoutes) HandleVerifyMagicLink(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		utils.RespondWithError(w, http.StatusMethodNotAllowed, "Method not allowed", nil)
		return
	}
	...
	// valid, userID, err := h.Auth.Repository.GetTokenRepository().ValidateToken(req.Token, types.MakicLinkToken)
